passports_details = """eyr:2028 iyr:2016 byr:1995 ecl:oth
...
# Użyjmy malego zbioru danych aby widzieć co nie działa
# passports_details="""pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
# hcl:#623a2f
...
passports_details = """eyr:1972 cid:100
...
passports_list = passports_details.split('\n\n')


def is_passport_correct(passpost_details):
  passport_data = {}

  details_list = passpost_details.split("\n")
  for details in details_list:
      subdetails = details.split(" ")
      for i in subdetails:
        elements = i.split(":")
        if len(elements) != 2:
          return False
        passport_data[elements[0]] = elements[1]


  required_keys = {"byr", "iyr", "eyr", "hgt", "hcl", "ecl","pid"}
  if len(required_keys.difference(passport_data.keys())) > 0:
    return False
  ...
  num_tests_passed = 0
  for required_key in int_contraints:
    int_val = int(passport_data[required_key])
    if int_val >= int_contraints[required_key][0] and int_val <= int_contraints[required_key][1]:
      num_tests_passed+=1
    else:
      return False
  ...
  # print(clear_details_list)

  #     Tym sposobem otrzymaliśmy w clean_details_list wyczysszczone listy poszczególnych paszportów,
  #     gdzie elementami są "klucz:wartość"

  existing_keys_list = []
  # print(clear_details_list)
  for element in clear_details_list:
      existing_keys_list.append(element[0:3])

  


  if not ("byr" in existing_keys_list and \
          "iyr" in existing_keys_list and \
          "eyr" in existing_keys_list and \
          "hgt" in existing_keys_list and \
          "hcl" in existing_keys_list and \
          "ecl" in existing_keys_list and \
          "pid" in existing_keys_list):
    return False

  passed_test = 0
  import re

  for detail in clear_details_list:
    elements = detail.split(":")
    if len(elements) != 2:
      return None
    ...
    if "pid" in detail:
        ...
        # print(pid[4:])
        regex = '^[0-9]{9}$'
        if re.match(regex, pid[4:]):
            # print(re.match(regex, pid[4:]))
            passed_test += 1
...
def passport_checker(passports):
    valid_documents = 0
    for passpost_details in passports:
        if is_passport_correct(passpost_details):
            valid_documents += 1
    print("Dokumenty prawilne: ", valid_documents)
    return valid_documents

passport_checker(passports_list)
