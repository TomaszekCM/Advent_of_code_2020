...
# musimy mieć:  1) listę z poszczególnymi literami hasła
# 2) listę z min i max wystąpień danej litery
# 3) daną literę (bez ":")
# Po czym musimy zwiększyć counter jeśli litera zawiera się więcej bądź równo razy niż mniejsza liczba i mniej
# lub równo razy niż druga liczba - tutaj trzeba jakiś podcounter stworzyć, co by lecąc po i-tym elemencie listy liter
# z hasła, zwiększał się jak jest to ta sama litera - dopiero ten podcounter trzeba porównać z zakresem

def correct_password(arg):
    counter = 0
    for i in arg:
        each_element = i.split()
        # each_element to jest lista z poszczególnych elementów każdej linijki

        range = each_element[0]
        # range to jest np "7-19"
        range_list = range.split("-")
        # każdy z 2 elementów jest teraz napisem, więc potem trzeba to zrobić int.
        # print(range_list)

        password = each_element[2]
        # wyodrębnione samo hasło
        pass_list = list(password)
        # hasło jako lista poszczególnych liter
        # print(pass_list)

        letter_plus = each_element[1]
        # wyodrębniona litera wraz z ":"
        letter = letter_plus.replace(':', '')
        # czysta litera, bez żadnych śmieci
        # print(letter)
        ...
        for n in pass_list:
            if letter == n:
                how_many_letters += 1
        # nasz podcounter pokazuje ile razy występuje żądana litera
        # print(how_many_letters)

        if how_many_letters >= int(range_list[0]) and how_many_letters <= int(range_list[1]):
            counter += 1

    print(counter)


correct_password(passwords_list)




def correct_password2(arg):
    counter = 0
    for i in arg:
        each_element = i.split()
        # each_element to jest lista z poszczególnych elementów każdej linijki

        positions = each_element[0]
        # range to jest np "7-19"
        positions_list = positions.split("-")
        # każdy z 2 elementów jese teraz napisem, więc potem trzeba to zrobić int.
        ...
        # sprawdzamy czy pierwsza litera jest równa tej właściwej
        # print("pierwsza: ", pass_list[int(positions_list[0])-1])
        if pass_list[int(positions_list[0])-1] == letter:
            # jeśli tak, to zwiększamy subcounter
            subcounter += 1

        # sprawdzamy czy druga litera jest równa właściwej
        # print("druga: ", pass_list[int(positions_list[1])-1])
        if pass_list[int(positions_list[1])-1] == letter:
            # jeśli tak, to zwiększamy subcounter
            subcounter += 1

        # print(subcounter)

        # jeśli jest tylko jedno poprawne trafienie, liczymy je
        if subcounter == 1:
            counter += 1

    print(counter)

correct_password2(passwords_list)