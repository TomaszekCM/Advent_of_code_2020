...
bags_info="""muted white bags contain 4 dark orange bags, 3 bright white bags.
...
# bag type -> list of tuples: (number, type of bag)
# for example
# bags_dict["shiny gold"] = [(1, "dark olive"), (2, "vibrant plum")]
bags_dict = {}

# bags_info ="""shiny gold bags contain 2 dark red bags.
...
for line in bags_info.splitlines():
    tokens = line.split(" bags contain ")
    bag_type = tokens[0]
    contained_bags = []
    for contained_bag_type in tokens[1].split(", "):
       elements = contained_bag_type.split(" ")
       if elements[0] == "no":
           break
       contained_bags.append((int(elements[0]), "%s %s" % (elements[1], elements[2])))
    bags_dict[bag_type] = contained_bags


# Odwrocmy bag_dict (klucz moze byc zawarty w torbach ktore pojawiaja sie jako wartosci)
# czyli dla kazdej torby zapisujemy w slowniku rodzaje toreb w ktorych ta torba moze sie znajdowac.
#
# jesli oryinalny slownik mial a->[b,c] to odwrocoy slownik ma:
# c->[a] b->[a]
bag_contained_in = {}
for bag_type in bags_dict:
  for quantity, contained_bag in bags_dict[bag_type]:
    if contained_bag not in bag_contained_in:
      bag_contained_in[contained_bag] = []
    bag_contained_in[contained_bag].append(bag_type)
    # print(bag_contained_in[contained_bag])

# print(bag_contained_in)

# Teraz zaczniemy od shining gold i bedziemy patrzyc w jakich torbach moze sie znalezc... a potem w jakich torbach tamte torby moga sie znalezc.

def get_bags_which_can_contain(bag_type):
  if bag_type not in bag_contained_in:
    return set()
  types_which_can_contain_current = set(bag_contained_in[bag_type])
  for containing_type in bag_contained_in[bag_type]:
    types_which_can_contain_current.update(get_bags_which_can_contain(containing_type))
  return types_which_can_contain_current

print(len(get_bags_which_can_contain("shiny gold")))

def how_many_bags_in_my_bag(my_bag):
  sum_of_bags = 0
  for bag in bags_dict[my_bag]:
    # print("%s -> %s" % (my_bag,  bag))
    sum_of_bags += bag[0]*(1+ (how_many_bags_in_my_bag(bag[1])))

  return sum_of_bags

# print(bags_dict["shiny gold"])

print("Suma toreb w mojej torbie: ", how_many_bags_in_my_bag("shiny gold"))